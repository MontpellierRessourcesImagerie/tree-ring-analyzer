...
if __name__ == "__main__":
    ...
    output_folder = '/home/khietdang/Documents/khiet/treeRing/tile'
    ...
    for im_name in os.listdir(input_folder):
        ...
        # mask = tifffile.imread(full_path.replace('input', 'masks'))
        ...
        for i in range(0, len(predictions)):
            # tifffile.imwrite(os.path.join(output_folder, im_name[:-3]) + '_' + str(i) + '.tif', predictions[i])
            plt.figure(figsize=(10, 5))
            plt.subplot(1, 2, 1)
            plt.imshow(tiles[i])
            plt.subplot(1, 2, 2)
            plt.imshow(predictions[i])
            plt.savefig(os.path.join(output_folder, im_name[:-3]) + '_' + str(i) + '.png')
        raise ValueError
